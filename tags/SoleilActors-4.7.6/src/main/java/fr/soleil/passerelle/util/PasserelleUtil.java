...
public final class PasserelleUtil{
	...
	public static Object getInputValue(ManagedMessage message) throws ProcessingException{
		try {
			...
			throw new ProcessingException("INPUT PORT ERROR - Cannot get input message", message, e);
		}
	}
	...
	public static ManagedMessage createContentMessage(Actor act, Object content) throws ProcessingException{
		 ...
		 try {
			...
			throw new ProcessingException("OUTPUT PORT ERROR - Error creating output message", content, e);
		}
		 return resultMsg;
	...
	public static ManagedMessage createCopyMessage(Actor act, ManagedMessage msg) throws ProcessingException{
		 ManagedMessage resultMsg;
		 try {
			 resultMsg = MessageFactory.getInstance().copyMessage(msg);
		} catch (MessageException e) {
			throw new ProcessingException("OUTPUT PORT ERROR - Error creating output message", msg, e);
		}
		 return resultMsg;
	...
	public static ManagedMessage createContentMessageInSequence(Actor act, Object content, Long sequenceID, long sequencePos, boolean isSequenceEnd) throws ProcessingException{
		ManagedMessage resultMsg = MessageFactory.getInstance().createMessageInSequence(sequenceID,
				sequencePos, isSequenceEnd);
		 try {
				resultMsg.setBodyContent(content,ManagedMessage.objectContentType);
			} catch (MessageException e) {
				throw new ProcessingException("OUTPUT PORT ERROR - Error creating output message", content, e);
