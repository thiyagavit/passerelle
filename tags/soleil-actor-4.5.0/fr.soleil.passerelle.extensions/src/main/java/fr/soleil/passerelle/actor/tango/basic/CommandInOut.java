...
public class CommandInOut extends Actor {
	...
	private final static Logger logger = LoggerFactory
			.getLogger(CommandInOut.class);
	/**
	 * If there is, the input argument of the command.
	 */
	public Port argin1;// = null;
	/**
	 * Used only for complex input argurments like Tango_DEVVAR_LONGSTRINGARRAY
	 * or Tango_DEVVAR_DOUBLESTRINGARRAY ( a mix of a table of string and a
	 * table of double or long) Will be visible only is parameter
	...
	/**
	 * Used only for complex output argurments like Tango_DEVVAR_LONGSTRINGARRAY
	 * or Tango_DEVVAR_DOUBLESTRINGARRAY ( a mix of a table of string and a
	 * table of double or long) Will be visible only is parameter
	...
	/**
	 * to usecomplex input argurments like Tango_DEVVAR_LONGSTRINGARRAY or
	 * Tango_DEVVAR_DOUBLESTRINGARRAY ( a mix of a table of string and a table
	 * of double or long)
	 */
	...
	/**
	 * to usecomplex output argurments like Tango_DEVVAR_LONGSTRINGARRAY or
	 * Tango_DEVVAR_DOUBLESTRINGARRAY ( a mix of a table of string and a table
	 * of double or long)
	 */
	 ...
	 */
	public CommandInOut(final CompositeEntity arg0, final String arg1)
		...
		argin1 = PortFactory.getInstance().createInputPort(this, "argin1",
				String.class);
		...
		useXStringArrayInParam = new Parameter(this,
				ARGIN_IS_A_TANGO_DEVVAR__X_STRINGARRAY, new BooleanToken(false));
		useXStringArrayInParam.setTypeEquals(BaseType.BOOLEAN);
		useXStringArrayInParam.setVisibility(Settable.EXPERT);

		useXStringArrayOutParam = new Parameter(this,
				ARGOUT_IS_A_TANGO_DEVVAR__X_STRINGARRAY,
				new BooleanToken(false));
		useXStringArrayOutParam.setTypeEquals(BaseType.BOOLEAN);
		useXStringArrayOutParam.setVisibility(Settable.EXPERT);

		final URL url = this.getClass().getResource(
				"/fr/soleil/tango/tango.jpg");
		_attachText(
				"_iconDescription",
				"<svg>\n"
						+ " <image x=\"-20\" y=\"-20\" width =\"40\" height=\"40\" xlink:href=\""
						+ url + "\"/>\n"
	...
	public void attributeChanged(final Attribute arg0)
		...
		if (arg0 == deviceNameParam) {
			...
			commandName = ((StringToken) commandNameParam.getToken())
					.stringValue();
		} else if (arg0 == useXStringArrayInParam) {
			useXStringArrayIn = new Boolean(useXStringArrayInParam
					.getExpression().trim());
			...
			if (useXStringArrayIn) {
				if (argin2 == null) {
					...
					try {
						...
						if (extraInputPort == null) {
							argin2 = PortFactory.getInstance().createInputPort(
									this, "argin2 (String)", String.class);
						} else {
				...
				if (argin2 != null) {
					...
					try {
						...
						throw new IllegalActionException(this, e,
								"Error removing argin2");
					}
				}
			}
		} else if (arg0 == useXStringArrayOutParam) {
			useXStringArrayOut = new Boolean(useXStringArrayOutParam
					.getExpression().trim());
				...
				if (argout2 == null) {
					try {
						...
						if (extraOutputPort == null) {
							argout2 = PortFactory.getInstance()
									.createOutputPort(this, "argout2 (String)");
						} else {
							argout2 = extraOutputPort;
						}
					} catch (final NameDuplicationException e) {
						throw new IllegalActionException(this, e,
								"Error adding argout2");
					}
				}
			} else {
				if (argout2 != null) {
	...
	protected void doInitialize() throws InitializationException {
		...
		if (argin1.getWidth() <= 0) {
			throw new InitializationException(
					PasserelleException.Severity.FATAL, "Input port "
							+ argin1.getName() + " has no data.", this, null);
		}

		if (useXStringArrayIn && argin2.getWidth() <= 0) {
			throw new InitializationException(
					PasserelleException.Severity.FATAL, "Input port "
							+ argin2.getName() + " has no data.", this, null);
		}
		...
		if (!isMockMode()) {
			try {
				...
				throw new InitializationException(
						...
						"Exception during TangoCommand " + e.getMessage(),
						this, null);
			}
			...
			if (argin2.getWidth() > 0) {
				if (logger.isTraceEnabled()) {
					logger.trace(getInfo()
							+ " doInitialize() -  new PortHandler(argin2)");
				}
	...
	protected void doFire() throws ProcessingException {
		...
		try {
			if (!tokenIsNull1 && !tokenIsNull2) {
				...
				if (isMockMode()) {
					ExecutionTracerService.trace(this, "MOCK - executing "
							+ commandName + " on device: " + deviceName);
					sendOutputMsg(argout1, PasserelleUtil
							.createTriggerMessage());
					if (argout2 != null) {
						sendOutputMsg(argout2, PasserelleUtil
								.createTriggerMessage());
					}
				} else {
					...
					if (comHelp.isArginScalar()) {
						// scalar
						logger.debug("scalar");
						comHelp.execute((String) PasserelleUtil
								.getInputValue(messageArgin1));
					} else if (comHelp.isArginSpectrum()) {
						// simple spectrum
						comHelp.execute(((String) PasserelleUtil
								.getInputValue(messageArgin1)).split(","));
					} else if (comHelp.isArginMixFormat()) {
						// mix spectrum
						comHelp.insertMixArgin(((String) PasserelleUtil
								.getInputValue(messageArgin1)).split(","),
								((String) PasserelleUtil
										.getInputValue(messageArgin2))
										.split(","));
						comHelp.execute();
					...
					if (!isFinishRequested()) {
						// -----------------extract
						// argout---------------------------
						if (comHelp.isArgoutScalar()
								|| comHelp.isArgoutSpectrum()) {
							// scalar
							final String val = comHelp.extractToString(",");
							sendOutputMsg(argout1, PasserelleUtil
									.createContentMessage(this, val));
						} else if (comHelp.isArgoutMixFormat()) {
							// mix spectrum
							final String[] numVal = comHelp
									.getNumMixArrayArgout();
							final String[] stringVal = comHelp
									.getStringMixArrayArgout();
							sendOutputMsg(argout1, PasserelleUtil
									.createContentMessage(this, numVal));
							sendOutputMsg(argout2, PasserelleUtil
									.createContentMessage(this, stringVal));
						} else {
				...
				if (logger.isTraceEnabled()) {
					logger.trace(getInfo() + " doFire() - request finish");
				}
				requestFinish();
			}
		} catch (final DevFailed e) {
			...
			throw new ProcessingException("", deviceName, e);
		...
		try {
			...
			if (useXStringArrayIn) {
				if (!isMockMode()) {
					...
					if (!comHelp.isArginMixFormat()) {
						throw new ProcessingException(
								"The parameter argin Tango_DEVVAR_<X>STRINGARRAY must be enable",
								deviceName, null);
					}
				}
				...
				if (!isMockMode()) {
					...
					if (comHelp.isArginMixFormat()) {
						throw new ProcessingException(
								"The parameter argin Tango_DEVVAR_<X>STRINGARRAY must be disable",
								deviceName, null);
					}
				}
			}
		} catch (final PasserelleException e) {
			throw new ProcessingException("cannot get input", deviceName, e);
		} catch (final DevFailed e) {
			throw new DevFailedProcessingException(e, this);
		}
