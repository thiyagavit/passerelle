...
public class MCAMultiport extends ATangoDeviceActor {
	...
	public MCAMultiport(final CompositeEntity container, final String name)
		...
		roi10Param = new StringParameter(this,
				"Roi10 start,end (sep by commas)");
		roi10Param.setExpression("0,0");

		roi11Param = new StringParameter(this,
				"Roi11 start,end (sep by commas)");
		roi11Param.setExpression("0,0");

		roi12Param = new StringParameter(this,
				"Roi12 start,end (sep by commas)");
		roi12Param.setExpression("0,0");

		roi13Param = new StringParameter(this,
				"Roi13 start,end (sep by commas)");
		roi13Param.setExpression("0,0");

		roi14Param = new StringParameter(this,
				"Roi14 start,end (sep by commas)");
		roi14Param.setExpression("0,0");

		roi15Param = new StringParameter(this,
				"Roi15 start,end (sep by commas)");
		roi15Param.setExpression("0,0");

		roi16Param = new StringParameter(this,
				"Roi16 start,end (sep by commas)");
		roi16Param.setExpression("0,0");

		roi17Param = new StringParameter(this,
				"Roi17 start,end (sep by commas)");
		roi17Param.setExpression("0,0");

		roi18Param = new StringParameter(this,
				"Roi18 start,end (sep by commas)");
		roi18Param.setExpression("0,0");

		roi19Param = new StringParameter(this,
				"Roi19 start,end (sep by commas)");
		roi19Param.setExpression("0,0");

		roi20Param = new StringParameter(this,
				"Roi20 start,end (sep by commas)");
	...
	public void attributeChanged(final Attribute arg0)
		...
		if (arg0 == nbChannelsParam) {
			nbChannels = Integer
					.valueOf(nbChannelsParam.getExpression().trim());
		} else if (arg0 == integrationTimeParam) {
			integrationTime = Double.valueOf(integrationTimeParam
					.getExpression().trim());
	...
	protected void doInitialize() throws InitializationException {
		...
		if (!isMockMode()) {
			try {
				...
				attInChannelsHelper = new TangoAttribute(deviceName
						+ "/NbChannels");
				attInTimeHelper = new TangoAttribute(deviceName
						+ "/integrationTime");
				attOutHelper = new TangoAttribute(deviceName + "/DataSpectrum");
			} catch (final DevFailed e) {
				final InitializationException initEx = new InitializationException(
	...
	protected void process(final ActorContext ctxt,
		...
		if (isMockMode()) {
			...
			try {
				...
				attOutHelper.update();
				response.addOutputMessage(0, output, PasserelleUtil
						.createContentMessage(this, attOutHelper));
			} catch (final DevFailed e) {
