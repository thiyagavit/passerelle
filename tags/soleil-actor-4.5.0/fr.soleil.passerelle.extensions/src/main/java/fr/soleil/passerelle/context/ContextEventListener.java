...
public class ContextEventListener extends Attribute implements
	FiringEventListener {
    ...
    public ContextEventListener(final Director container, final String name)
	    throws IllegalActionException, NameDuplicationException {
	super(container, name);
	container.registerFiringEventListener(this);
	...
	if (container.getAttribute(CONTEXT_STRATEGY_NAME) == null) {
	    final Parameter param = new StringParameter(container,
    ...
    private String getStringFromPtolemyAttribute(final FiringEvent event,
	...
	try {
	    ...
	    if (deviceName == null) {
		deviceName = SoleilUtilities
			.getDevicesFromClass("BeamlineStatus")[0];
	    }
	    try {
	...
	if (event.getType().equals(FiringEvent.BEFORE_FIRE)) {
	    final String contextName = getStringFromPtolemyAttribute(event,
		    CONTEXT_NAME);
	    final String contextStrategy = getStringFromPtolemyAttribute(event,
		    CONTEXT_STRATEGY_NAME);
    ...
    /**
     * check context to know if it has to pause. Synchronized because onEvent is
     * called by several threads
     * 
     * @param e
     * @param contextName
     */
    private synchronized void pause(final FiringEvent e,
	    final String contextName) {
	boolean contextValidity = false;
	boolean firtTurn = true;
	final CompositeActor model = (CompositeActor) e.getDirector()
		.getContainer();
	if (model.getManager().getState() != Manager.WRAPPING_UP) {
		...
		if (contextAlive) {
			...
			logger.error(TangoToPasserelleUtil.getDevFailedString(e2,
				(Actor) null));
		    }
		} else {
		    if (firtTurn) {
			ExecutionTracerService.trace(
				(com.isencia.passerelle.domain.cap.Director) e
					.getDirector(),
				"ERROR - context device is not alive");
		    }
		}
		if (!contextValidity) {
	    ...
	    } while (!contextValidity
		    && model.getManager().getState() != Manager.WRAPPING_UP);
	}
    }
	...
	if (model.getManager().getState() != Manager.WRAPPING_UP) {
		...
		ExecutionTracerService.trace(
			(com.isencia.passerelle.domain.cap.Director) e
				.getDirector(), "STOPPING - CONTEXT INVALID");
