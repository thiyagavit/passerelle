...
public class WaitScalarAttribute extends ATangoAttributeActor {
     ...
     */
    public WaitScalarAttribute(final CompositeEntity container, final String name)
	...
	waitReadPartParam = new Parameter(this, WAIT_READ_PART_EQUALS_WRITE_PART, new BooleanToken(
		true));
	...
	if (isMockMode()) {
	    if (waitReadPart) {
		ExecutionTracerService.trace(this, "MOCK - waiting for " + getAttributeName()
			+ " equals to write part");
	    } else {
		ExecutionTracerService.trace(this, "MOCK - waiting for " + getAttributeName()
			+ " equals to " + waitValue);
	    }
		...
		if (waitReadPart) {
		    ExecutionTracerService.trace(this, "waiting for " + getAttributeName()
			    + " equals to write part");
		    waitTask = new WaitAttributeTask(attr, tolerance, timeout, 1000, null, false);
		} else {
		    ExecutionTracerService.trace(this, "waiting for " + getAttributeName()
			    + " equals to " + waitValue);
		    waitTask = new WaitAttributeTask(attr, tolerance, timeout, 1000, waitValue,
			    true);
