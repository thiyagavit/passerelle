...
public class SpectrumPeakFinder extends Actor {
	...
	public SpectrumPeakFinder(final CompositeEntity arg0, final String arg1)
		...
		positionX = PortFactory.getInstance().createOutputPort(this,
				"positionX");
				...
				public void tokenReceived() {
					...
					if (token != null) {
						try {
							...
							logger.debug("yHandler.tokenReceived() :"
	...
	public void attributeChanged(final Attribute attribute)
		...
		if (attribute == maxPercentParam) {
			maxPercent = new Double(((StringToken) maxPercentParam.getToken())
					.stringValue());
		}
		...
		if (!tokenIsNull) {
			...
			if (xVect.size() != yVect.size()) {
				throw new ProcessingException(
						"The two input tables must have the same size", null,
						null);
			}

			// find the max
			final Double max = Collections.max(yVect);
			final double maxWithPercent = max.doubleValue()
					* maxPercent.doubleValue() / 100;
			...
			/*
			 * System.out.println("------");
			 * System.out.println("index of max is "+ yVect.indexOf(max));
			 */
			// Double positionXval = (Double)xVect.get(pos);
			/*
			 * System.out.println("position of max is "+ positionXval);
			 * System.out.println("-------");
