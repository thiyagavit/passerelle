...
public abstract class MotorMover extends ATangoDeviceActor {
    ...
    public MotorMover(final CompositeEntity container, final String name,
	...
	if (isMockMode()) {
		...
		if (!port.equals(errorPort) && !port.getName().equals("hasFired")
		    ...
		    response.addOutputMessage(0, port, PasserelleUtil.createContentMessage(this,
			    desiredPosition));
		}
		i++;
	    }
		...
		for (final Port port : outputPortList) {
		    // FIXME: find another way to retrieve default ports
		    if (!port.getName().equals(errorPort.getName())
			    && !port.getName().equals("hasFired")
			    && !port.getName().equals("hasFinished")) {
			final TangoAttribute attHelper = attrList.get(i);
			attHelper.update();
			logger.debug("output " + attHelper.getAttributeProxy().fullName() + " on "
				+ port.getFullName());
			response.addOutputMessage(0, port, PasserelleUtil.createContentMessage(
				this, attHelper));
			i++;
		    }
		}
