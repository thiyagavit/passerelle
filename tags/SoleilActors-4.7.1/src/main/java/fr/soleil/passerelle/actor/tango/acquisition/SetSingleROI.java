...
public class SetSingleROI extends ATangoDeviceActor {
    ...
    public SetSingleROI(final CompositeEntity container, final String name)
	...
	if (isMockMode()) {
	    ExecutionTracerService.trace(this, "MOCK - Setting ROI " + roiIndex
		    + " from " + startROI + " to " + endROI);
	} else {
	    try {
		ExecutionTracerService.trace(this, "Setting ROI " + roiIndex
			+ " from " + startROI + " to " + endROI);
		setSingleROI.execute(Double.toString(roiIndex), Double
			.toString(startROI), Double.toString(endROI));
	    } catch (final DevFailed e) {
