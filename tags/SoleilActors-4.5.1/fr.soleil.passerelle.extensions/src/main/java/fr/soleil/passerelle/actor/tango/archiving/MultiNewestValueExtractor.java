...
public class MultiNewestValueExtractor extends AArchivingExtractor {
    ...
    public MultiNewestValueExtractor(final CompositeEntity container, final String name) throws IllegalActionException,
		...
		if (isMockMode()) {
			...
			ExecutionTracerService.trace(
				...
				attributeName + " extraction is : [timestamp= "
					+ DateUtils.format(date, "yyyy/MM/dd HH:mm:ss") + ",value " + i + "]");
