...
public class Apple2UndulatorMover extends MotorMover {

	static Vector<String> attributeList = new Vector<String>();
	static {
		attributeList.add("energy");
		attributeList.add("polarisation");
		attributeList.add("gap");
		attributeList.add("phase");
	}

	public Apple2UndulatorMover(final CompositeEntity container,
			final String name) throws NameDuplicationException,
			IllegalActionException {
		super(container, name, attributeList);
		final URL url = this.getClass().getResource("/image/BL_Undulator.png");
		_attachText(
				"_iconDescription",
				"<svg>\n"
