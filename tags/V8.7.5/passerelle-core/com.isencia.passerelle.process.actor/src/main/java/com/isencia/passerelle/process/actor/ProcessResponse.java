...
/**
 * In the new Passerelle Actor API, the ProcessResponse is a generic container for all response messages, and the ports on which they should be sent, that are
 * generated by an actor by processing a received ProcessRequest. <br>
...
public class ProcessResponse {
  ...
  public boolean addOutputContext(MessageOutputContext msgCtxt) {
    if (request.isMessageInContext(msgCtxt.getMessage())) {
      outputs.add(msgCtxt);
      return true;
    } else {
      return false;
  ...
  public boolean addOutputContextInSequence(MessageOutputContext msgCtxt) {
    if (request.isMessageInContext(msgCtxt.getMessage())) {
