...
public class WaitState extends ATangoDeviceActor {
	 ...
	 */
	public WaitState(final CompositeEntity container, final String name) throws NameDuplicationException, IllegalActionException {
	...
	protected void process(final ActorContext ctxt, final ProcessRequest request, final ProcessResponse response) throws ProcessingException {
		...
		if (isMockMode()) {
			ExecutionTracerService.trace(this, "MOCK - waiting state: " + DeviceState.toString(waitingState) + " on " + getDeviceName());
