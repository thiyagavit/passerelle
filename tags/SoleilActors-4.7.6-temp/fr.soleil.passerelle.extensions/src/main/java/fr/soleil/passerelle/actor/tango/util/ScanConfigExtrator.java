...
public class ScanConfigExtrator extends Transformer{
	...
	public ScanConfigExtrator(CompositeEntity container, String name) throws NameDuplicationException, IllegalActionException {
	...
	protected void doFire(ManagedMessage message) throws ProcessingException {
		...
		if(isMockMode()){
			...
			try {
				...
				throw new ProcessingException("Cannot connect to scan servers",
						config, e1);
			}			
	...
	if (attribute == fileNameParam) 
		fileName = fileNameParam.asFile().getPath();
	else 
