...
public class Synchronizer extends Actor {
	 ...
	 */
	public Synchronizer(final CompositeEntity container, final String name)
			throws IllegalActionException, NameDuplicationException {
		super(container, name);
		syncInput = PortFactory.getInstance().createInputPort(this,
				"syncInput", null);
	...
	protected void doInitialize() throws InitializationException {
		...
		if (syncInput.getWidth() > 0) {
			...
			throw new InitializationException("syncInput is not connected",
					syncInput, null);
		}

		for (final Port port : inputPorts) {
			if (port.getWidth() == 0) {
				throw new InitializationException(port.getName()
						+ " is not connected", port, null);
			}
		}
		...
		if (token != null) {
			...
			if (!finishRequest) {
				for (int i = 0; i < nrPorts; ++i) {
					...
					if (msg != null) {
						System.out.println("not null- output data for "
								+ outputPort.getName());
	...
	public void attributeChanged(final Attribute attribute)
		...
		if (logger.isTraceEnabled()) {
			logger.trace(getInfo()
					+ " attributeChanged() - entry - attribute :" + attribute);
		}

		// Change numberOfOutputs
		if (attribute == numberOfPorts) {
			int nrPorts = inputPorts.size();
			final int newPortCount = ((IntToken) numberOfPorts.getToken())
					.intValue();
				...
				for (int i = nrPorts; i < newPortCount; ++i) {
					try {
						...
						if (extraInputPort == null) {
							extraInputPort = PortFactory.getInstance()
									.createInputPort(this, inputPortName, null);
						...
						if (extraOutputPort == null) {
							extraOutputPort = PortFactory.getInstance()
									.createOutputPort(this, outputPortName);
						}
