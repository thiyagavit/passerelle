...
public class InsertExtractPositioner extends ATangoDeviceActor {
	...
	public InsertExtractPositioner(final CompositeEntity arg0, final String arg1)
	...
	protected void process(final ActorContext ctxt,
		...
		try {
			if (isMockMode()) {
				if (action.equalsIgnoreCase("Insert")) {
					ExecutionTracerService.trace(this, "MOCK - inserting "
							+ getDeviceName());
					ExecutionTracerService.trace(this, "MOCK - "
							+ getDeviceName() + " is inserted");
				} else {
					ExecutionTracerService.trace(this, "MOCK - extracting "
							+ getDeviceName());
					ExecutionTracerService.trace(this, "MOCK - "
							+ getDeviceName() + " is extracted");
				}
			} else {
				comHelp.execute();
			...
			response.addOutputMessage(0, output, MessageFactory.getInstance()
					.createTriggerMessage());
