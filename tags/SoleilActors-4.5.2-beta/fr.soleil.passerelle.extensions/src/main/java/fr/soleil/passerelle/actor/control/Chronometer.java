 ...
 * 
 * If a reset message has already arrived before a set message, the reset will
 * be immediately noticed after the set, and the resulting time interval will be
 * approximately 0.
 * 
 * If another set message arrives before the reset, it means that immediately
 * after consuming the reset, in the next fire() iteration, the actor will be
 * set again.
 * 
 * So, the Chronometer does not drop any messages and it will almost
 * continuously be blocked on one of the 2 input ports.
 * 
...
public class Chronometer extends Transformer {
	...
	public Chronometer(final CompositeEntity container, final String name)
		...
		// null);
		input.setName("set");
		resetInputPort = PortFactory.getInstance().createInputPort(this,
				"reset", null);
	...
	protected void doFire(final ManagedMessage arg0) throws ProcessingException {
		...
		try {
			...
			throw new ProcessingException("can't get input message",
					resetInputPort, e1);
		}
		...
		message = PasserelleUtil.createContentMessage(this, Double
				.toString(totalTime));
	 ...
	 */
	@Override
	protected String getAuditTrailMessage(final ManagedMessage message,
			final Port port) throws Exception {
		return "sent chronometer message with time "
