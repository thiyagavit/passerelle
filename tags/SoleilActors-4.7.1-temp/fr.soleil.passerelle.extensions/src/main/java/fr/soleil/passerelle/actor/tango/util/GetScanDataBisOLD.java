...
public class GetScanDataBisOLD extends Transformer {
    ...
    public GetScanDataBisOLD(final CompositeEntity container, final String name)
	...
	if (isMockMode()) {
		...
		for (int i = 1; i < actuatorsNb + 1; i++) {
			...
			throw new ProcessingException("Actuator " + i
				+ " does not exists on current scan", i, null);
		    }
