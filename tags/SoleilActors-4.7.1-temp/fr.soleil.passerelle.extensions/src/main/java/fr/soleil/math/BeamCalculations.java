...
public class BeamCalculations {
	...
	public static double[] getHmPositions(final double[] x, final double[] y) {
		...
		final double x1 = Analysis.linearInterpolation(halfMax, x1Down, x1Up,
				y1Down, y1Up);
		final double x2 = Analysis.linearInterpolation(halfMax, x2Down, x2Up,
				y2Down, y2Up);

		return new double[] { x1, x2 };
	}

	public static double fwhm(final double[] x, final double[] y) {
	 ...
	 */
	public static double intersectionPosition(final double[] x,
			final double[] y, final double[] initialGuess)
		...
		try {
			...
			final double parameters[] = fitter.fit(
					new BeamCalculations.FunctionForFittingDualRamp(),
					initialGuess);
			logger.debug(Arrays.toString(parameters));
			logger.debug("MaxIterations: " + optimizer.getMaxIterations()
					+ ", Iterations " + optimizer.getIterations());
			logger.debug("MaxEvaluations "
					+ optimizer.getMaxEvaluations() + ", Evaluations "
					+ optimizer.getEvaluations());
			logger.debug("RMS " + optimizer.getRMS());
			for (final double element : x) {
				logger.debug(String.valueOf(new BeamCalculations.FunctionForFittingDualRamp().value(element, parameters)));
			}
			return parameters[0];
	 ...
	 * p: pente de la 2eme droite <br>
	 */
	public static class FunctionForFittingDualRamp implements
