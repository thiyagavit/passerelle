...
public class EquipementsSetter extends Transformer {
    ...
    public EquipementsSetter(final CompositeEntity arg0, final String arg1)
	...
	if (isMockMode()) {
	    ExecutionTracerService.trace(this,
		    "MOCK - setting equipments with snapshot " + snapShotID
			    + " and " + type);
	} else {
	    try {
		cmd.execute(snapShotID, type);
		ExecutionTracerService.trace(this,
			"setting equipments with snapshot " + snapShotID
				+ " and " + type);
	    } catch (final DevFailed e) {
