...
public class GetScanDataOLD extends Transformer {
    ...
    public GetScanDataOLD(final CompositeEntity container, final String name)
	...
	if (!isMockMode()) {
		...
		throw new InitializationException(
			"Cannot connect to scan server", null, e);
	    ...
	    if (scanServer == null) {
		throw new InitializationException(
			"Cannot connect to scan server", null, null);
	    }
	}
	super.doInitialize();
    }
	...
	if (isMockMode()) {
		...
		if (port.getName().startsWith(SENSOR)
		    ...
		    sendOutputMsg(port, PasserelleUtil.createContentMessage(
			    this, "1,2,3"));
		}
	    }
		...
		for (int i = 0; i < actuators.length; i++) {
			...
			throw new ProcessingException("Actuator "
				+ actuators[i]
				+ " does not exists on current scan",
				actuators[i], null);
		    }
		    ...
		    // read attribute is done by the TangoAttribute constructor
		    ExecutionTracerService.trace(this,
			    "reading data for actuator " + actuators[i]);
		    sendOutputMsg(orderedActuatorPorts.get(i), PasserelleUtil
			    .createContentMessage(this, act));
		}
		for (int i = 0; i < sensors.length; i++) {
		    scanDataName = sensorsSources.get(sensors[i]);
		    if (scanDataName == null) {
			throw new ProcessingException("Sensor " + sensors[i]
				+ " does not exists on current scan",
				sensors[i], null);
		    }
		 ...
		 // read attribute is done by the TangoAttribute constructor
		    ExecutionTracerService.trace(this,
			    "reading data for sensor " + sensors[i]);
		    sendOutputMsg(orderedSensorPorts.get(i), PasserelleUtil
			    .createContentMessage(this, sensor));
		}
	    } catch (final DevFailed e) {
