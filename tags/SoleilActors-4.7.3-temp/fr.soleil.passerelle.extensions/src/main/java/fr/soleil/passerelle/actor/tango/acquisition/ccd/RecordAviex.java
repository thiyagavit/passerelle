...
public class RecordAviex extends ATangoDeviceActor {
    ...
    public RecordAviex(final CompositeEntity arg0, final String arg1)
	...
	if (!isMockMode()) {
		...
		selectFrameAttr = new TangoAttribute(getDeviceName()
			+ "/selectFrame");
		...
		try {
		    ...
		    throw new InitializationException("Flag must be numerical",
			    flagValue, e);
		}
	    }
	}
	...
	if (isMockMode()) {
	    ExecutionTracerService.trace(this,
		    "MOCK - Starting recording Aviex data");
	    ExecutionTracerService.trace(this,
		    "MOCK - Aviex data has been saved");
	} else {

	    try {
		ExecutionTracerService
			.trace(this, "Waiting flag for recording");
