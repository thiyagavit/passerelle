...
public class WaitStateTask extends CancellableTangoTask {
	 ...
	 */
	public WaitStateTask(final DeviceProxy dev, final DevState state, final int pollingPeriod, final boolean waitForState) {
		this.dev = dev;
		this.state = state;
	 ...
	 */
	public WaitStateTask(final DeviceProxy dev, final DevState state, final int pollingPeriod, final boolean waitForState, final double timeOut, final String errorMsg, final String orginErrorMsg) {
		this.dev = dev;
		this.state = state;
		this.pollingPeriod = pollingPeriod;
		this.waitForState = waitForState;
	...
	public void run() {
		...
		try {
			do {
				...
				stateCondition = waitForState ? !currentState.equals(state) : currentState.equals(state);
