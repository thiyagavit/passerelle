...
/**
 * Executes Passerelle Models. the incoming message may contain the list of
 * "models" (files names) separated by commas.
 *
 * @author Dirk Jacobs
 */
...
public class ModelExecutor extends Actor {
	...
	public ModelExecutor(CompositeEntity container, String name)
		...
		trigger = PortFactory.getInstance().createInputPort(this,
				"models names", null);
		outputTrigger = PortFactory.getInstance().createOutputPort(this,
				"trigger");
		modelsPaths = null;
	...
	protected void doFire() throws ProcessingException {
			...
			if (token == null) {
				...
				try {
					...
					throw new ProcessingException(
							getInfo()
									+ " - doFire() generated an exception while reading message",
							token, e);
				}
			...
			if ((modelPath != null) && (modelPath.length > 0)) {
				...
				for (int i = 0; i < modelPath.length; i++) {
					try {
						...
						CompositeActor topLevel = (CompositeActor) parser
								.parse(null, xmlFile);
						if (getAuditLogger().isInfoEnabled()) {
							getAuditLogger().info("Executing " + modelPath[i]);
						}
						logger.debug("Model Executor - Executing "
								+ modelPath[i]);
						...
						throw new ProcessingException(getInfo()
								+ " - doFire() generated an exception " + e,
								null, e);
					}
				}
			}
		...
		ManagedMessage resultMsg = MessageFactory.getInstance()
		...
		try {
			...
			throw new ProcessingException("Error sending output data",
					outputTrigger, e2);
		}
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.isencia.passerelle.actor.Actor#doPostFire()
	 */
	@Override
