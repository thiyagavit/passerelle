...
public class Diffractometer extends Actor {
	private static final String[] HKL_COORDINATE = { "HCoordinate",
			"KCoordinate", "LCoordinate" };
	...
	public Diffractometer(final CompositeEntity container, final String name)
		...
		// Ports
		hCoordinatePort = PortFactory.getInstance().createInputPort(this,
				"HCoordinate", Double.class);
		hCoordinatePort.setMultiport(false);
		kCoordinatePort = PortFactory.getInstance().createInputPort(this,
				"KCoordinate", Double.class);
		kCoordinatePort.setMultiport(false);
		lCoordinatePort = PortFactory.getInstance().createInputPort(this,
				"LCoordinate", Double.class);
	...
	protected void doInitialize() throws InitializationException {
		...
		if (hCoordinatePort.getWidth() > 0) {
			...
			hCoordinateHandler = new PortHandler(hCoordinatePort,
					new PortListener() {
						public void tokenReceived() {
							...
							if (token != null) {
								try {
									...
									hCoordinateConst = (Double) message
											.getBodyContent();
		...
		if (kCoordinatePort.getWidth() > 0) {
			...
			kCoordinateHandler = new PortHandler(kCoordinatePort,
					new PortListener() {
						public void tokenReceived() {
							...
							if (token != null) {
								try {
									final ManagedMessage message = MessageHelper
											.getMessageFromToken(token);
									kCoordinateConst = (Double) message
											.getBodyContent();
								} catch (final MessageException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
		...
		if (lCoordinatePort.getWidth() > 0) {
			...
			lCoordinateHandler = new PortHandler(lCoordinatePort,
					new PortListener() {
						public void tokenReceived() {
							...
							if (token != null) {
								try {
									final ManagedMessage message = MessageHelper
											.getMessageFromToken(token);
									lCoordinateConst = (Double) message
											.getBodyContent();
								} catch (final MessageException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
	...
	protected void process(final ActorContext ctxt,
		...
		while (!hCoordinateReceived || !kCoordinateReceived
				|| !lCoordinateReceived) {
			performWait(1000);
		}
		hCoordinateReceived = false;
			...
			try {
				if (isMockMode()) {
					ExecutionTracerService.trace(this,
							"MOCK - start moving H/K/L on " + deviceName);
					ExecutionTracerService.trace(this,
							"MOCK - end of moving H/K/L on " + deviceName);
				} else {
					...
					ExecutionTracerService.trace(this,
							"end of moving H/K/L on " + deviceName);
				}
				sendOutputMsg(output, MessageFactory.getInstance()
						.createTriggerMessage());
			} catch (final DevFailed e) {
				throw new ProcessingException("Initialization problem", this,
						null);
			}
