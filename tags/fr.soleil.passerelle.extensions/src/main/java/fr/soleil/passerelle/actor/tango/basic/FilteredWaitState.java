...
public class FilteredWaitState extends ATangoDeviceActor {
	 ...
	 */
	public FilteredWaitState(final CompositeEntity container, final String name) throws NameDuplicationException, IllegalActionException {
	...
	protected void doInitialize() throws InitializationException {
		...
		logger.debug(getInfo() + " doInitialize() - entry");
		...
		super.doInitialize();
		logger.debug(getInfo() + " doInitialize() - exit");
	...
	protected void process(final ActorContext ctxt, final ProcessRequest request, final ProcessResponse response) throws ProcessingException {
		...
		logger.debug(getInfo() + " doFire() - entry");

		if (isMockMode()) {
			ExecutionTracerService.trace(this, "MOCK - waiting state: " + DeviceState.toString(waitingState));
			ExecutionTracerService.trace(this, "MOCK - waiting state: " + DeviceState.toString(waitingState) + " finished");
		} else {
			try {
				...
				waitState = new WaitStateOnGroupTask(devGroupProxy, waitingState, 1000, timeOut, "All devices nerver change their state to " + DeviceState.toString(waitingState),
						"FilteredWaitState");
				waitState.run();
