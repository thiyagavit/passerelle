...
public class MultiValueExtractor extends AArchivingExtractor {
  ...
  public MultiValueExtractor(final CompositeEntity container, final String name) throws IllegalActionException, NameDuplicationException {
  ...
  protected void process(final ActorContext ctx, final ProcessRequest request, final ProcessResponse response) throws ProcessingException {
    ...
    try {
      ...
      for (final String attributeName : attributeNames) {
        if (isMockMode()) {
          command.execute(attributeName, new DevVarDoubleStringArray(new double[] { 10 }, new String[] { attributeName }));
        } else {
