...
public class MultipleOutputsSynchronizer extends DynamicPortsActor {
    private static Logger logger = LoggerFactory.getLogger(MultipleOutputsSynchronizer.class);
     ...
     */
    public MultipleOutputsSynchronizer(final CompositeEntity container, final String name)
	...
	// output data only if alls inputs has received something
	if (inputProcessed == getInputPorts().size()) {
	    allInputContexts = request.getAllInputContexts();
	    while (allInputContexts.hasNext()) {
		final MessageInputContext messageInputContext = allInputContexts.next();
		if (!messageInputContext.isProcessed()) {
