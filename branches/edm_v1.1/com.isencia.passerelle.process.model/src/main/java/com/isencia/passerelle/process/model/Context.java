...
public interface Context extends Identifiable, Serializable {
  ...
  /**
   * A <code>repositoryID</code> is different from the <code>id</code> which is typically a PK generated by a persistent store
   * for each persistent instance. 
   * <p>
   * Transient <code>Context</code> instances don't have (unique) PKs. E.g. forked <code>Context</code>s typically share their <code>id</code>
   * with their parent (pre-fork) <code>Context</code>.
   * </p>
   * But ALL instances do have a unique <code>repositoryID</code> with which they can be identified in the runtime's <code>ContextRepository</code>
   * during their processing.
   * <p>
   * Usage patterns for <code>id</code> and <code>repositoryId</code> are also sufficiently different to justify their existence.
   * <ul>
   * <li>The PK is typically a formatted sequential number that can be used for easy DB querying, partitioning etc.</li>
   * <li>The <code>repositoryId</code> is a technical internal key, used by the runtime to manage executing processing flows.</li>
   * </ul>
   * </p>
   * 
