...
public class MoveDownStepAction extends AbstractMultipleSelectionAction {

    /**
     *
     */
    public MoveDownStepAction(BossaNovaSequencerImpl sequencer, JTable table) {
        ...
        // constructor of the superclass is invoked
            ...
            // Set tool tip text
            putValue(Action.SHORT_DESCRIPTION, "Move down Step");

            // This text is not directly used by any Swing component;
            // however, this text could be used in a help system
            putValue(Action.LONG_DESCRIPTION, "Move down Step");

            // Set an icon
            putValue(Action.SMALL_ICON, Icons.getIcon("bossanova.list.move.down"));
            ...
            putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_DOWN,InputEvent.SHIFT_DOWN_MASK+InputEvent.CTRL_DOWN_MASK));
    }

    
    public void doSpecificUnitaryActionOnRow(int row) {
		if (row >= 0) {
			sequencer.moveStepdown(row);
		}
