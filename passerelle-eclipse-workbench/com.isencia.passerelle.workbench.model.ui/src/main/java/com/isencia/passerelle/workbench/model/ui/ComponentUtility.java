...
public abstract class ComponentUtility {
  ...
  public static List<Command> deleteConnections(NamedObj model) {
    ...
    Iterator<?> sourceIterator = ModelUtils.getConnectedRelations(model, ConnectionType.SOURCE, true).iterator();
    while (sourceIterator.hasNext()) {
      List<Command> cmds = generateDeleteCommand(model, (IORelation) sourceIterator.next());
      for (Command cmd : cmds) {
        cmd.execute();
        delecteConnectionCommands.add(cmd);
      }
    }
    Iterator<?> targetIterator = ModelUtils.getConnectedRelations(model, ConnectionType.TARGET, true).iterator();
    while (targetIterator.hasNext()) {
      List<Command> cmds = generateDeleteCommand(model, (IORelation) targetIterator.next());
      for (Command cmd : cmds) {
  ...
  private static List<Command> generateDeleteCommand(NamedObj model, IORelation relation) {
    ...
    if (!ModelUtils.containsVertex(relation)) {
      ...
      for (IOPort port : ModelUtils.getPorts(relation, model)) {
        cmds.add(generateDeleteVertexConnectionCommand(model, relation, port));
      }
    }
    return cmds;
  }

  private static DeleteConnectionCommand generateDeleteConnectionCommand(Nameable actor, IORelation relation) {
  ...
  private static DeleteVertexConnectionCommand generateDeleteVertexConnectionCommand(Nameable actor, IORelation relation, Port port) {
