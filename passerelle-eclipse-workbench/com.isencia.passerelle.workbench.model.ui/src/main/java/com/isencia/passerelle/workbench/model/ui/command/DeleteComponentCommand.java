...
public class DeleteComponentCommand extends Command implements IRefreshConnections {
  ...
  /*
   * private void detachFromGuides(LogicSubpart part) { if (part.getVerticalGuide() != null) { vGuide =
   * part.getVerticalGuide(); vAlign = vGuide.getAlignment(part); vGuide.detachPart(part); } if
   * (part.getHorizontalGuide() != null) { hGuide = part.getHorizontalGuide(); hAlign = hGuide.getAlignment(part);
  ...
  private void restoreRelation(NamedObj child, List destination, List source) {
    try {
      if (!source.isEmpty() && !destination.isEmpty()) {
        CreateConnectionCommand connection = new CreateConnectionCommand((ComponentPort) source.get(0), (ComponentPort) destination.get(0), multiPageEditor);
