...
public class VariableEditingSupport extends EditingSupport {
  ...
  /**
   * NOTE: the attribute may now implement 'CellEditorParameter' in which case this method is not asked for the cell
   * editor. This allows exotic editors to be created without changing the core passerelle code.
   * 
   * @param parameter
   * @param type
   * @return
  ...
  public PropertyDescriptor getPropertyDescriptor(final Attribute parameter, final Type type) {
    ...
    if (parameter instanceof ColorAttribute) {
      ...
      return new TextPropertyDescriptor(parameter.getName(), parameter.getDisplayName());
    }
