...
public abstract class ASnapExtractor extends Transformer {
  ...
  public ASnapExtractor(final CompositeEntity container, final String name) throws NameDuplicationException, IllegalActionException {
    ...
    final URL url = this.getClass().getResource("/org/tango-project/tango-icon-theme/32x32/devices/camera-photo.png");
    _attachText("_iconDescription", "<svg>\n" + "<rect x=\"-20\" y=\"-20\" width=\"40\" " + "height=\"40\" style=\"fill:orange;stroke:black\"/>\n"
  ...
  protected void doInitialize() throws InitializationException {
    if (isMockMode()) {
      ...
      try {
        extractor = new SnapExtractorProxy(getSnapExtractorName());
      } catch (final DevFailed e) {
        throw new DevFailedInitializationException(e, this);
      }
    }
  ...
  /**
   * Retrieve read and write values from snap for a list of attributes. Send them to the 2 output ports
   * 
   * @throws DevFailed
  ...
  protected void getAndSendValues() throws DevFailed, ProcessingException {
    if (isMockMode()) {
      ExecutionTracerService.trace(this, "MOCK - snap values for snap ID " + snapID + " : [read = " + 1 + "] [write = " + 2 + "]");
...
//        switch (extractionType) {
//        case READ:
//          ExecutionTracerService.trace(this, name + " snap values for snap ID " + snapID + " : [read = " + snapReadValues[i] + "]");
...
//          break;
//        case WRITE:
//          ExecutionTracerService.trace(this, name + " snap values for snap ID " + snapID + " : [write = " + snapWriteValues[i] + "]");
...
//          break;
//        default:
//          ExecutionTracerService.trace(this, name + " snap values for snap ID " + snapID + " : [read = " + snapReadValues[i] + "] [write = "
//              + snapWriteValues[i] + "]");
