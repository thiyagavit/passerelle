...
public class ForLoop extends Actor {
	...
	public ForLoop(final CompositeEntity container, final String name)
		...
		startValueParam = new StringParameter(this, "Start Value");
		startValueParam.setExpression("0");

		endValueParam = new StringParameter(this, "End Value");
		endValueParam.setExpression("3");

		stepWidthParam = new StringParameter(this, "Step Width");
	...
	protected void doInitialize() throws InitializationException {
		...
		if (handledPort.getWidth() > 0) {
			...
			handledPortHandler = new PortHandler(handledPort,
					new PortListener() {
		...
		final BigDecimal totalWidth = end
				.subtract(start, MathContext.DECIMAL32).abs();
		final BigDecimal div = totalWidth.divide(new BigDecimal(stepWidth),
	...
	public void attributeChanged(final Attribute arg0)
		...
		if (arg0 == startValueParam) {
			startValue = Double.valueOf(((StringToken) startValueParam
					.getToken()).stringValue());
		} else if (arg0 == stepWidthParam) {
			stepWidth = Double
					.valueOf(((StringToken) stepWidthParam.getToken())
							.stringValue());
			if (stepWidth <= 0) {
				throw new IllegalActionException(stepWidthParam,
						"Step Width must be positive");
			}
		} else if (arg0 == endValueParam) {
			endValue = Double.valueOf(((StringToken) endValueParam.getToken())
					.stringValue());
	...
	protected void doFire() throws ProcessingException {
		...
		if (inputMsg != null) {
			...
			while (!handledPortExhausted && currentStep < stepNumber + 1
					&& !isFinishRequested()) {
				try {
	...
	private void sendLoopData() throws ProcessingException {
		...
		try {
			resultMsg.setBodyContent(new Double(currentValue),
