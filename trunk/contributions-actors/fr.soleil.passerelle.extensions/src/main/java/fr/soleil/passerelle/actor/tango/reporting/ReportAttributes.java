...
public class ReportAttributes extends ATangoDeviceActor {
    ...
    public ReportAttributes(final CompositeEntity container, final String name)
	...
	if (isMockMode()) {
		...
		ExecutionTracerService.trace(this,
			"no device found for pattern: " + deviceName);
	    }
		...
		if (!started) {
			...
			final Pattern pattern = Pattern.compile(FilterHelper
				.wildcardToRegex(attributeName),
				Pattern.CASE_INSENSITIVE);
			for (final String element : attrList) {
			    logger.debug("***test matching for " + device + "/"
				    + element);
			    final Matcher matcher = pattern.matcher(element);
			    if (matcher.matches()) {
				logger.debug("read " + device + "/" + element);
				try {
				    final TangoAttribute attr = new TangoAttribute(
					    device + "/" + element);
				    attributeVal = attr.readAsString("", "");
				    ExecutionTracerService.trace(this, device
					    + "/" + element + " value: "
					    + attributeVal);
				} catch (final DevFailed e) {
				    ExecutionTracerService.trace(this, device
