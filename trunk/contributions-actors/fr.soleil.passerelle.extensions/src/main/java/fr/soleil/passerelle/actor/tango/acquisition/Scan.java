...
public class Scan extends TransformerV5 implements IActorFinalizer{
	 ...
	 */
	public Scan(final CompositeEntity container, final String name)
	...
	public void validateInitialization() throws ValidationException {
		...
		if (!isMockMode()) {
			...
			try {
				conf = (ConfigImpl<?>) ScanUtil.getCurrentSalsaApi()
						.getConfigByPath(confName);
	...
	protected void process(final ActorContext ctxt,
		...
		if (isMockMode()) {
			ExecutionTracerService.trace(this,
					"MOCK - Scan started with config: " + confName);
			ExecutionTracerService.trace(this, "MOCK - Scan finished");
		} else {
			ExecutionTracerService.trace(this, "Scan started with config: "
					+ confName);
			...
			if (scanTask.hasFailed()) {
				throw new ProcessingExceptionWithLog(this, Severity.FATAL,
						scanTask.getError().getMessage(), this, scanTask
								.getError());
			} else {
