...
public class Switch extends Actor {
	...
	public Switch(final CompositeEntity container, final String name)
		...
		select = PortFactory.getInstance().createInputPort(this, "select",
				String.class);
		input.setMultiport(false);
	...
	public void attributeChanged(final Attribute attribute)
		...
		if (attribute == numberOfOutputs) {
			final int newOutputCount = ((IntToken) numberOfOutputs.getToken())
					.intValue();
			logger.debug("change number of outputs from :  " + outputCount
					+ " to : " + newOutputCount);
			...
			if (outputPorts == null) {
				...
				for (int i = 0; i < newOutputCount; i++) {
					try {
						...
						if (outputPort == null) {
							outputPort = PortFactory.getInstance()
									.createOutputPort(this, OUTPUT + i);
						}
	...
	protected void doFire() throws ProcessingException {
		...
		if (select.getWidth() == 0 && input.getWidth() == 0) {
			...
			if (token == null) {
				...
				if (selected < 0) {
					outNr = 0;
					logger.debug(getInfo() + " : Selected port = " + selected
							+ ". Using port " + outNr + ".");
					ExecutionTracerService.trace(this, "Selected port = "
							+ selected + ". Using port " + outNr + ".");
				...
				try {
					ExecutionTracerService.trace(this, "Selected port = "
							+ outNr);
					sendOutputMsg(orderedPorts.get(outNr), PasserelleUtil
							.createCopyMessage(this, MessageHelper
		...
		if (logger.isTraceEnabled()) {
			logger.trace(getInfo() + " - exit " + " - Output " + outNr
					+ " has sent message " + token);
		}
	}
	...
	protected void doInitialize() throws InitializationException {
		...
		// Only one port is connected
			...
			throw new InitializationException(select.getName() + " port and "
					+ input.getName() + " have to be connected", (select
					.getWidth() == 0) ? select : input, null);
		}
