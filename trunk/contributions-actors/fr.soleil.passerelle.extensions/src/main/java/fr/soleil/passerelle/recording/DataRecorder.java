...
public class DataRecorder {
    ...
    public synchronized void setAsyncMode(final String dataRecorderName,
    ...
    public synchronized void saveDevice(final Actor actor,
	    final String deviceName) throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
		final boolean autoChangeNX = ((RecordingDirector) actor
			.getDirector()).isAutoChangeNxEntry();
		if (autoChangeNX) {
		    incNxEntryNameAndSaveContext(actor);
		}
		final String dataRecorderName = ((RecordingDirector) actor
			.getDirector()).getDataRecorderName();
		ExecutionTracerService.trace(actor, "saving device "
			+ deviceName + " on datarecorder " + dataRecorderName);
		final TangoCommand command = new TangoCommand(dataRecorderName,
    ...
    public synchronized void saveExperimentalData(final Actor actor,
	    final String deviceName) throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
		...
		ExecutionTracerService.trace(actor,
			"saving experimental data for " + deviceName
				+ " on datarecorder " + dataRecorderName);
		final TangoCommand command = new TangoCommand(dataRecorderName,
     ...
     */
    @Deprecated
    public synchronized void saveScan(final Actor actor,
	    final String scanDeviceName, final String scan1DDeviceName,
	...
	if (dir instanceof RecordingDirector) {
		...
		final String dataRecorderName = ((RecordingDirector) actor
			.getDirector()).getDataRecorderName();
		ExecutionTracerService.trace(actor,
			"saving scan data on datarecorder " + dataRecorderName);
		final TangoCommand command = new TangoCommand(dataRecorderName,
			"WriteScanData");

		final String[] devicesNames = { scanDeviceName,
			scan1DDeviceName, scan2DDeviceName };
    ...
    public synchronized void setSymbol(final Actor actor,
	    final String symbolName, final String symbolValue) throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
		final String dataRecorderName = ((RecordingDirector) actor
			.getDirector()).getDataRecorderName();
		logger.debug("Setting symbol " + symbolName + " on "
			+ dataRecorderName);
		final TangoCommand command = new TangoCommand(dataRecorderName,
			"SetSymbol");
	...
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    final String deviceName = ((RecordingDirector) actor.getDirector())
		    .getDataRecorderName();
	    final TangoCommand comHelp = new TangoCommand(deviceName,
		    "StartRecording");
	    comHelp.execute();
	} else {
	    ExecutionTracerService.trace(actor,
		    "WARNING - there is no Recording director");
	}
    }

    public synchronized void endRecording(final String dataRecorderName)
	    throws DevFailed {
	if (isRecordingStarted(dataRecorderName)) {
	    final TangoCommand comHelp = new TangoCommand(dataRecorderName,
		    "EndRecording");
	    comHelp.execute();
	}
    }
	...
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
		final String deviceName = ((RecordingDirector) actor
			.getDirector()).getDataRecorderName();
		final TangoCommand comHelp = new TangoCommand(deviceName,
			"EndRecording");
		comHelp.execute();
    ...
    public synchronized void incNxEntryNameAndSaveContext(final Actor actor)
	    throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
		...
		// increment indexes
		final TangoCommand commandIncAcq = new TangoCommand(
			dataRecorderName, "IncAcquisitionIndex");
		commandIncAcq.execute();

		final TangoCommand commandExpAcq = new TangoCommand(
			dataRecorderName, "IncExperimentIndex");
		commandExpAcq.execute();

		// write user data
		// System.out.println("user data ");
		final TangoCommand commandWriteUserData = new TangoCommand(
			dataRecorderName, "WriteUserData");
		...
		final TangoCommand commandWritePreTechnicalData = new TangoCommand(
			dataRecorderName, "WritePreTechnicalData");
    ...
    public synchronized void setNxEntryNameAndSaveContext(final Actor actor,
	    final String suffix) throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
		final String dataRecorderName = ((RecordingDirector) actor
		...
		final TangoAttribute acqName = new TangoAttribute(
			dataRecorderName + "/acquisitionName");
    ...
    public synchronized void savePostContext(final Actor actor)
	    throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    if (isRecordingStarted(actor)) {
    ...
    public synchronized boolean isRecordingStarted(final String dataRecorderName)
	    throws DevFailed {
    ...
    public synchronized boolean isRecordingStarted(final Actor actor)
	    throws DevFailed {
	final Director dir = actor.getDirector();
	if (dir instanceof RecordingDirector) {
	    final String dataRecorderName = ((RecordingDirector) actor
