...
public class StringsAdder extends Actor {
    ...
    public StringsAdder(final CompositeEntity container, final String name)
    ...
    public void attributeChanged(final Attribute attribute) throws IllegalActionException {
        if (attribute == separatorParam) {
            ...
            stringNumber = Integer.valueOf(((StringToken) stringNumberParam.getToken())
                    .stringValue());
    ...
    protected void doFire() throws ProcessingException {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (port.getWidth() > 0) {
                ...
                throw new ProcessingException(PasserelleException.Severity.FATAL, "Input port "
                        + port.getName() + " has no data.", this, null);
            }
        }// For
        if (!finishRequested) {
