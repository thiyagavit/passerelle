...
public class MathFunction extends Actor {
    /**
     * The function to compute. This is a string-valued attribute that defaults
     * to "exp".
     */
    ...
    /**
     * The port for the second operand, if it is needed. The port has type
     * BaseType.DOUBLE
     */
	...
	firstOperand = PortFactory.getInstance().createInputPort(this, "input",
		Double.class);
	...
	if (token != null) {
		...
		resultMsg.setBodyContent(
			new Double(_doFunction(input1, input2)),
			ManagedMessage.objectContentType);
		output.broadcast(new ObjectToken(resultMsg));
	    } catch (final MessageException e) {
		throw new ProcessingException("Cannot send result to output",
			output, e);
     ...
     * @throws
     */
    private double getDoubleFromMessage(final Token token)
	    throws ProcessingException {
	...
	try {
		...
		result = Double.parseDouble(((TangoAttribute) input)
			.readAsString("", ""));
	    } else {
	    ...
	    throw new ProcessingException("Error reading msg content", input, e);
	} catch (final DevFailed e) {
	    e.printStackTrace();
	    throw new ProcessingException(
		    TangoToPasserelleUtil.getDevFailedString(e, this), input, e);
	}
	return result;
    }
	...
	if (secondOperand == null) {
	    secondOperand = PortFactory.getInstance().createInputPort(this,
		    "secondOperand", Double.class);
