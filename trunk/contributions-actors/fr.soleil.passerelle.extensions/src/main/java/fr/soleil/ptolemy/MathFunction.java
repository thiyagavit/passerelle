...
/**
 * @author root TODO To change the template for this generated type comment go to Window - Preferences - Java - Code Style - Code Templates
 */
...
public class MathFunction extends Actor {
   ...
   */
  public MathFunction(final CompositeEntity container, final String name) throws NameDuplicationException, IllegalActionException {
  ...
  protected void doFire() throws ProcessingException {
    ...
    if (token != null && token != Token.NIL) {
      ...
      try {
        ...
        throw new ProcessingException("Cannot send result to output", output, e);
  ...
  private double getDoubleFromMessage(final Token token) throws ProcessingException {
    ...
    try {
      ...
      e.printStackTrace();
      throw new ProcessingException(TangoToPasserelleUtil.getDevFailedString(e, this), input, e);
    }
    return result;
  }
  ...
  private double _doFunction(final double input1, final double input2) {
    ...
    case _SQRT:
      ...
      break;
    default:
      throw new InternalErrorException("Invalid value for _function private variable. " + "MathFunction actor (" + getFullName() + ")" + " on function type "
          + _function);
    }
    return result;
  }
