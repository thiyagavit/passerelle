 ...
 * </p>
 * 
 * TODO split value is now set as a <code>Context</code> entry. I.e. it is not effectively replacing the original item which could be in any <code>ResultBlock</code>,
 * <code>Request</code> attribute etc. Check if this is OK in all required use cases.
 * (i.e. in rules modules, facts are typically constructed based on result blocks etc, and the context entries are not necessarily taken into account I think!?
 * 
 * @author erwin
 */
