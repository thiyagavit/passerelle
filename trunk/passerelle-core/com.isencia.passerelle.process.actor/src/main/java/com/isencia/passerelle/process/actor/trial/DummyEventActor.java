...
import com.isencia.passerelle.process.model.Event;
 ...
 * <p>
 * Each blank line represents a break in the event stream. I.e. the actor will
 * send the events before the break right after each other, and then wait a bit
 * (configurable interval) before sending the next ones.
 * 
 * </p>
 * 
 * @author delerw
...
public class DummyEventActor extends Actor {
	 ...
	 */
	public DummyEventActor(CompositeEntity container, String name) throws IllegalActionException,
			...
			while ((eventDef = reader.readLine()) != null) {
				if (eventDef.trim().length() == 0) {
					...
					if (eventFields.length == 3) {
						try {
							ResultItem e = createEvent(eventFields[0], eventFields[1], eventFields[2]);
							currentBatch.add(e);
						} catch (Exception e) {
						...
						ExecutionTracerService.trace(this,
								"Invalid event definition: wrong nr of fields (should be 3) : " + eventDef);
					}
				}
			}
		} catch (Exception e) {
	...
	private ResultItem createEvent(String date, String name, String value) throws Exception {
		SimpleDateFormat fmt = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		Date d = fmt.parse(date);
		return ServicesRegistry.getInstance().getDiagnosisEntityFactory()
				.createResultItem(name, value, (String) null, d, null);
	}
