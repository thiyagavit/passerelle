...
public class BossaNovaSequencerImpl extends Observable implements Runnable, Sequencer {
    ...
    private void resetRunningStepIndex() {
        ...
        if (isStopRequested()) {
            ...
            BossaNovaData.getSingleton().getApplication().getTraceComponent()
                    .trace((Director) null, stopMsg);
        }
        BossaNovaData.getSingleton().getApplication().getMainScreen().getDirectorPanel()
                .setVisible(true);
    ...
    // Bug 17641
    /***
     * private void displayViewComp(Flow flow, Step step) { MainScreen mainScreen =
     * BossaNovaData.getSingleton().getApplication().getMainScreen(); // TODO QUICK HACK // if
     * (entity instanceof ISpecificBeans) { // ISpecificBeans beans = (ISpecificBeans) entity; //
    ...
    private void executeStep(final int stepIndex, final Step step) throws Exception {
        ...
        if (validateStep(step)) {
             ...
             * flow.setDirector(batch.getBatchDirector()); // step.applyFlowParametersToStep(); }
             * else { // Fixing Checkstyle bug JC Pret flow = createFlow(step, false); // End JC
             * Pret }
             ***/
            ...
            try {
                ...
                for (int i = 0; i < step.getIterationCount(); i++) {
                    ...
                    BossaNovaData
                            ...
                            .trace((Director) null,
                                    "Executing " + step.getName() + "("
                                            + (currentRunningStepIndex + 1) + ") - iteration "
                                            + (i + 1) + " on " + step.getIterationCount());
                    ...
                    BossaNovaData.getSingleton().getApplication()
                            .launchModel(flow, modelExecutor, true, false);
                    // if stop is requested we need to stop execution
                    if (isStopRequested()) {
                        return;
                    }
            ...
            if (step != null && step.getName() != null && !step.isBlockDelimiter()) {
                BossaNovaData
                        ...
                        .getTraceComponent()
                        .trace((Director) null,
                                step.getName() + "(" + (currentRunningStepIndex + 1)
                                        + ") is NOT executing");
            }
        }
    }
    ...
    /**
     * This method creates a new "template" batch director. Each creation of a FLow for an
     * actor-type step should then assign a clone of this director, using the method
     * getBatchDirectorClone().
     * 
     * @return
     * @throws Exception
     */
            ...
            if (stepName != null && !"".equals(stepName.trim()) && !step.isBlockDelimiter()) {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(BossaNovaData.getSingleton().getApplication()
                            .getDialogHookComponent(), "Error loading " + step.getType() + " "
                            + step.getName());
                    step.setOnFault(true);
                    // throw(e);
                }
                // End Bug 17624
                flowManager
                        .applyParameterSettings(result, step.getParameters(), step.getContexts());
            }
        } else {
    ...
    private void connectAllActorInputs(final Flow flow, final TypedAtomicActor actor) {
        if (actor instanceof Actor) {
            for (final Iterator<IOPort> iterator = actor.inputPortList().iterator(); iterator
                    .hasNext();) {
    ...
    private void connectPortWithConstant(final Flow flow, final IOPort inputPort) {
        try {
            ...
            if (!(inputPort instanceof ControlPort)) {
                if (inputPort != null) {
                    final ptolemy.actor.Actor trigger = flow.addActor(
                            com.isencia.passerelle.actor.general.Const.class, inputPort.getName());
                    final IOPort triggerOutput = (IOPort) ((ComponentEntity) trigger)
                            .getPort("output");
     ...
     */
    @Override
    public boolean addSequenceStep(final String stepName, final String comment,
