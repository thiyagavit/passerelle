...
public class PaletteItemFactory implements Serializable {
  ...
  /**
   * HACK warning: class name to palette item map
   * 
   * But the same class may be used in more than one actor. For now each actor in the palette must have its own class.
   * This is because only the class is contained in the moml file. To fix it would require a moml change to include id.
   * Instead just use unique classes for now.
   */
  ...
  public CombinedTemplateCreationEntry createPaletteEntryFromPaletteDefinition(PaletteItemDefinition def) {
    if (def instanceof SubModelPaletteItemDefinition) {
      return new CombinedTemplateCreationEntry(def.getName(), def.getName(), new ClassTypeFactory(def.getClazz(), (SubModelPaletteItemDefinition) def), def.getIcon(), //$NON-NLS-1$
          def.getIcon()//$NON-NLS-1$
      );
    } else {
      return new CombinedTemplateCreationEntry(def.getName(), def.getName(), new ClassTypeFactory(def.getClazz(), def.getName()), def.getIcon(), //$NON-NLS-1$
          def.getIcon()//$NON-NLS-1$
      );
    }
...
//    }
    ...
    if (repositoryService != null) {
      String[] submodels = repositoryService.getAllSubmodels();
  ...
  public void addSubModel(String name) throws Exception {

    if (!paletteItemMap.containsKey(name)) {
      SubModelPaletteItemDefinition item = new SubModelPaletteItemDefinition(null, composites, name, name);
      paletteItemMap.put(name, item);
    }
  }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // logger.error("Cannot load "+ele.getAttribute("class"),
          // e1);
          return null;
        }
      }
