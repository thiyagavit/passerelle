...
public class Analysis {
	...
	private class BeamDataFunction implements UnivariateRealFunction {
		...
		public BeamDataFunction(final double[] x, final double[] y) {
			try {
				...
				logger.debug("interpolated by function "
						+ function.getClass());
	...
	public static double[] trapezoidIntegrale(final double[] x, final double[] y)
			throws ProcessingException {
		final Analysis s = new Analysis();
		final UnivariateRealFunction function = s.new BeamDataFunction(x, y);
		final UnivariateRealIntegrator integrator = new TrapezoidIntegrator(
				function);
		try {
			...
			throw new ProcessingException("impossible to integrate", null, e);
		} catch (final FunctionEvaluationException e) {
			throw new ProcessingException("impossible to integrate", null, e);
	...
	public static double linearInterpolation(final double intermediateVal,
			final double neighbor1Pos, final double neighbor2Pos,
			final double neighbor1Val, final double neighbor2Val) {
		final double a = (neighbor2Val - neighbor1Val)
				/ (neighbor2Pos - neighbor1Pos);
		final double b = neighbor1Val - a * neighbor1Pos;
		return (intermediateVal - b) / a;
	}

	public static double dividedDifferenceInterpolation(final double xVal,
			final double x[], final double y[]) throws ProcessingException {
		return Analysis.interpolate(new DividedDifferenceInterpolator(), xVal,
				x, y);
	}

	public static double splineInterpolation(final double xVal,
			final double x[], final double y[]) throws ProcessingException {
		return Analysis.interpolate(new SplineInterpolator(), xVal, x, y);
	}

	public static double nevilleInterpolation(final double xVal,
			final double x[], final double y[]) throws ProcessingException {
		return Analysis.interpolate(new NevilleInterpolator(), xVal, x, y);
	}

	private static double interpolate(
			final UnivariateRealInterpolator interpolator, final double xVal,
			final double x[], final double y[]) throws ProcessingException {
		UnivariateRealFunction function;
		try {
			function = interpolator.interpolate(x, y);
			...
			throw new ProcessingException("impossible to create interpolator",
					null, e);
		}
		try {
			return function.value(xVal);
		} catch (final FunctionEvaluationException e) {
			throw new ProcessingException("impossible to interpolate", null, e);
		}
	}

	public static double[] polynomialSplineDerivative(final double[] x,
			final double[] y) throws ProcessingException {
		...
		final UnivariateRealFunction derivateFunction = ((PolynomialSplineFunction) function)
				.derivative();
		final double[] result = new double[x.length];
		for (int i = 0; i < x.length; i++) {
			try {
				...
				throw new ProcessingException(
						"impossible to get derivative value", null, e);
			}
		}
		return result;
	}
	...
	public static double[] numericalDerivative(final double[] x,
			final double[] y) throws ProcessingException {
		final int size = x.length;
		if (x.length != y.length) {
			throw new ProcessingException(
					"input arrays must have the same size", null, null);
		}
		...
		result[size - 1] = (y[size - 1] - y[size - 2])
				/ (x[size - 1] - x[size - 2]);
		return result;
	}
