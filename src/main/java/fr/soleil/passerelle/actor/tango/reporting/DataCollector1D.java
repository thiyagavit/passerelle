...
public class DataCollector1D extends ATangoAttributeActor implements
    IActorFinalizer {
    ...
    public synchronized List<String> createList(final String attributeName) {
        ...
        final List<String> newList = Collections
    ...
    public synchronized void clearAll() {
        if (!hasBeenCleared) {
        for (final Map.Entry<String, List<String>> list : createdLists
            .entrySet()) {
            list.getValue().clear();
            // add attribute name to command argin
    ...
    public DataCollector1D(final CompositeEntity container, final String name)
    ...
    try {
        final String attrName = getTangoAttribute().getAttributeProxy()
            .name();
    ...
    try {
        ...
        newVal.remove(0);
        publishSpectrumAttr.writeSpectrum(newVal.toArray(new String[] {}));
