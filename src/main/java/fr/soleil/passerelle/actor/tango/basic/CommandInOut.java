...
public class CommandInOut extends Actor {
  ...
  /**
   * Used only for complex input argurments like Tango_DEVVAR_LONGSTRINGARRAY or Tango_DEVVAR_DOUBLESTRINGARRAY ( a mix
  ...
  /**
   * Used only for complex output argurments like Tango_DEVVAR_LONGSTRINGARRAY or Tango_DEVVAR_DOUBLESTRINGARRAY ( a mix
   * of a table of string and a table of double or long) Will be visible only is parameter
   ...
   */
  public CommandInOut(final CompositeEntity arg0, final String arg1) throws NameDuplicationException, IllegalActionException {
  ...
  protected void doInitialize() throws InitializationException {
    ...
    if (argin1.getWidth() <= 0) {
      throw new InitializationException(PasserelleException.Severity.FATAL, "Input port " + argin1.getName() + " has no data.", this, null);
    }

    if (useXStringArrayIn && argin2.getWidth() <= 0) {
      throw new InitializationException(PasserelleException.Severity.FATAL, "Input port " + argin2.getName() + " has no data.", this, null);
    }
    ...
    if (!isMockMode()) {
      try {
        ...
        throw new InitializationException(PasserelleException.Severity.FATAL, "Exception during TangoCommand " + e.getMessage(), this, null);
      }
    ...
    if (argin2 != null) {
      if (argin2.getWidth() > 0) {
        if (logger.isTraceEnabled()) {
          logger.trace(getInfo() + " doInitialize() -  new PortHandler(argin2)");
        }
      ...
      if (!tokenIsNull1 && !tokenIsNull2) {
        ...
        if (isMockMode()) {
          ...
          if (comHelp.isArginScalar()) {
            ...
            // simple spectrum
            comHelp.execute(((String) PasserelleUtil.getInputValue(messageArgin1)).split(","));
      ...
      if (useXStringArrayIn) {
        if (!isMockMode()) {
          ...
          if (!comHelp.isArginMixFormat()) {
            throw new ProcessingException("The parameter argin Tango_DEVVAR_<X>STRINGARRAY must be enable", deviceName, null);
          }
        }
        ...
        if (!isMockMode()) {
          ...
          if (comHelp.isArginMixFormat()) {
            throw new ProcessingException("The parameter argin Tango_DEVVAR_<X>STRINGARRAY must be disable", deviceName, null);
          }
        }
      }
