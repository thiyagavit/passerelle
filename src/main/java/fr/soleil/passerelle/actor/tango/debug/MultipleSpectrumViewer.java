...
public class MultipleSpectrumViewer extends Sink {
	...
	private AttributeMultipleSpectrumPanel pan;
	 ...
	 */
	public MultipleSpectrumViewer(final CompositeEntity container,
	...
	protected void doInitialize() throws InitializationException {
		...
		if (isMockMode()) {
			...
			pan = new AttributeMultipleSpectrumPanel();
		}
		super.doInitialize();
	}
	...
	protected void sendMessage(final ManagedMessage outgoingMessage)
		...
		if (isMockMode()) {
			...
			try {
				...
				TangoAttribute ap = (TangoAttribute) outgoingMessage
						.getBodyContent();
				if (logger.isTraceEnabled()) {
					logger.trace(getInfo() + ""
							+ ap.getAttributeProxy().fullName());
				}
				pan.setAttributeXName(ap.getAttributeProxy().fullName());

				final String[] ys = new String[inputsPorts.size()];
				final List<Port> portList = PortUtilities.getOrderedInputPorts(
						this, Y, 1);
				for (int i = 0; i < portList.size(); i++) {
					...
					if (!(message.getBodyContent() instanceof TangoAttribute)) {
						className = message.getBodyContent().getClass()
								.getName();
						throw new ProcessingException(
								"Input message must of type AttributeProxy (not "
										+ className + ")", this.getName(), null);
					}
					ap = (TangoAttribute) message.getBodyContent();
					ys[i] = ap.getAttributeProxy().fullName();
				}

				pan.setAttributeYNames(ys);
						...
						try {
							...
							if (extraInputPort == null) {
								extraInputPort = PortFactory.getInstance()
										.createInputPort(this, intputPortName,
												null);
							}
