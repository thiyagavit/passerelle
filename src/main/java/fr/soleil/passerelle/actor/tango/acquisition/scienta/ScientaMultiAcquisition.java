...
public class ScientaMultiAcquisition extends ATangoDeviceActor {
	...
	public ScientaMultiAcquisition(final CompositeEntity container,
		...
		doNextPort = PortFactory.getInstance().createInputPort(this, "doNext",
				null);
		// output.setName("End");

		endSingleAcqPort = PortFactory.getInstance().createOutputPort(this,
				"EndSingleAcquisition");
	}

	@Override
	public void attributeChanged(final Attribute param)
	...
	protected void process(final ActorContext ctxt,
		...
		firstTurn = true;
		final double excitationEnergy = (Double) PasserelleUtil
			...
			while (!doNextPortExhausted && continu) {
				try {
					...
					throw new ProcessingException(
							"impossible to get doNext input", doNextPort, e);
				}
			}
		} else {
			while (continu) {
		...
		response.addOutputMessage(1, output, PasserelleUtil
				.createTriggerMessage());
	}
	...
	private boolean doAcquisition() throws ProcessingException {
		...
		try {
			if (isMockMode()) {
				...
				if (manager.isActive() && continu) {
					ExecutionTracerService.trace(this,
							"MOCK - Starting Acquisition for config: "
									+ manager.getCurrentScientaRegion()
											.getRegionName());
					ExecutionTracerService.trace(this,
							"MOCK - Acquisition Finished");
				}
			} else {
				continu = manager.configureMulti(false);
				if (manager.isActive() && continu) {
					ExecutionTracerService.trace(this,
							"Starting Acquisition for config: "
									+ manager.getCurrentScientaRegion()
											.getRegionName());
					if (manager.getCurrentScientaRegion().getAcqMode()
							.compareToIgnoreCase("Swept") == 0) {
			...
			throw new ProcessingException("impossible to record data", null, e);
		}
		return continu;
	}
