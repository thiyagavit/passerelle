...
public class AcquisitionPrincetonOde extends CCDAcquisitionPerformer {
	...
	public AcquisitionPrincetonOde(final CompositeEntity arg0, final String arg1)
	...
	protected void process(final ActorContext ctxt,
		...
		if (isMockMode()) {
			...
			try {
				switch (AcquisitionPrincetonOde.acqModeMap.get(ccd.getConfig()
						.getAcqMode())) {
				case 0:
				case 2:
	...
	private void doIntensity0Acquisition() throws DevFailed {
		...
		final TangoCommand cmd = new TangoCommand(ccd.getDeviceName(),
				"AcquireIntensity0");
		cmd.execute();
	}

	private void doAverageIntensity0Acquisition() throws DevFailed,
			IllegalActionException {
		ExecutionTracerService.trace(this, "Acquire average I0");
		ccd.doStandardAcquisitionWithoutStore();
		final TangoCommand cmd = new TangoCommand(ccd.getDeviceName(),
				"SetAverageIntensity0");
		cmd.execute();
	}
