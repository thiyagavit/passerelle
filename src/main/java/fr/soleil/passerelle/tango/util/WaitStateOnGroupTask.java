...
public class WaitStateOnGroupTask extends CancellableTangoTask {
	 ...
	 */
	public WaitStateOnGroupTask(final Group group, final DevState state, final int pollingPeriod, final double timeOut, final String errorMsg, final String originErrorMsg) {
		this.group = group;
		this.state = state;
		this.pollingPeriod = pollingPeriod;
			...
			do {
				try {
					...
					if (replyList.has_failed()) {
						Except.throw_exception("TANGO_COMMUNICATION_ERROR", "impossible to execute State", "TangoUtil.waitForStateOnGroup");
					}
