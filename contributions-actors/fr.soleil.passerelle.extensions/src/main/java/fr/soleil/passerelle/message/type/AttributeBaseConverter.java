 ...
 */
public class AttributeBaseConverter implements
		com.isencia.passerelle.message.type.TypeConverter {
	static Logger logger = LoggerFactory
			.getLogger(AttributeBaseConverter.class);

	public Token convertPasserelleTokenToPtolemyToken(
			final PasserelleToken passerelleMsgToken, final Type targetType)
		...
		if (passerelleMsgToken != null) {
			if (passerelleMsgToken instanceof PasserelleToken
					&& isTargetTypeCompatible(targetType)) {
				Token result = null;
				final ManagedMessage passerelleMsg = MessageHelper
						.getMessageFromToken(passerelleMsgToken);
				try {
					if (passerelleMsg.getBodyContent() instanceof Event) {
						final Event event = (Event) passerelleMsg
								.getBodyContent();
						result = convertEventToSimpleTypedToken(event,
								targetType);

					} else {
						final TangoAttribute attribute = (TangoAttribute) passerelleMsg
								.getBodyContent();
						result = convertAttributeToSimpleTypedToken(attribute,
								targetType);
					}
					...
					throw new PasserelleException(
							"Unexpected error during conversion attempt",
	...
	public PasserelleToken convertPtolemyTokenToPasserelleToken(
			final Token origToken, final Class targetType)
			throws UnsupportedOperationException, PasserelleException {
		...
		if (origToken == null) {
			...
			try {
				passerelleMsg = MessageHelper
						.getMessageFromToken(passerelleMsgToken);
				if (passerelleMsg.getBodyContent() instanceof Event) {
					final Event event = (Event) passerelleMsg.getBodyContent();
					final Object resultContent = convertEventToSimpleTypedObject(
							event, targetType);
					passerelleMsg.setBodyContent(resultContent,
							ManagedMessage.objectContentType);
					logger
							.debug(" convertPtolemyTokenToPasserelleToken() - conversion done - PasserelleToken");

				} else {
					final TangoAttribute attribute = (TangoAttribute) passerelleMsg
							.getBodyContent();
					final Object resultContent = convertAttributeToSimpleTypedObject(
				...
				// this converter
				logger
						.debug(" convertPtolemyTokenToPasserelleToken() - UnsupportedOperationException");
				throw new UnsupportedOperationException();
			} catch (final PasserelleException e) {
				logger
						.debug(" convertPtolemyTokenToPasserelleToken() - PasserelleException");
				throw e;
			} catch (final Exception e) {
				logger
						.debug(" convertPtolemyTokenToPasserelleToken() - Exception");
				throw new PasserelleException(
						"Unexpected error during conversion attempt",
						passerelleMsgToken, e);
			}
	...
	/**
	 * Converts a Tango Attribute to a std Ptolemy token with the desired
	 * content type.
	 * 
	 * @param content
	 * @param targetType
	 * @return
	 */
	public Token convertAttributeToSimpleTypedToken(
			final TangoAttribute content, final Type targetType)
			throws UnsupportedOperationException, PasserelleException {
		...
		try {
			...
			if (content.isScalar()) {
				try {
					result = type.getTokenForString(content
							.extract(String.class));
					if (result == null) { // means that this attribute is WRITE
						// only
						result = type.getTokenForString(content
								.extractWritten(String.class));
					}
				} catch (final IllegalActionException e) {
					throw new PasserelleException(
							"Error during construction of Token", content, e);
				}
			} else {
				...
				try {
					...
					throw new PasserelleException(
							"Error during construction of ArrayToken", content,
							e);
				}
			}
			return result;
		} catch (final DevFailed e) {
			throw new PasserelleException(TangoToPasserelleUtil
					.getDevFailedString(e, (Director) null), content, e);
		}
	}
	 ...
	 * @return
	 */
	public Token convertEventToSimpleTypedToken(final Event event,
			final Type targetType) throws UnsupportedOperationException,
			PasserelleException {
		logger.debug("convertAttributeToSimpleTypedToken");
		final PtolemyType type = PtolemyType.getPtolemyType(targetType);
		Token result;
		try {
			if (event instanceof ResultItem) {
				result = type
						.getTokenForString(((ResultItem) event).getValue());
			} else {
				result = type.getTokenForString(event.toString());
			}

		} catch (final IllegalActionException e) {
			throw new PasserelleException("Error during construction of Token",
					event, e);
		}
		return result;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Object convertEventToSimpleTypedObject(final Event event,
			final Class targetType) throws UnsupportedOperationException,
			PasserelleException {
		Object result;
	...
	/**
	 * Converts a Tango Attribute to an object with the desired type (Double,
	 * String etc).
	 * 
	 * @param content
	 * @param targetType
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Object convertAttributeToSimpleTypedObject(
			final TangoAttribute content, final Class targetType)
			throws UnsupportedOperationException, PasserelleException {
		Object result;
		logger.debug("convertAttributeToSimpleTypedObject");
		try {
			if (Event.class.isAssignableFrom(targetType)) {
				result = new ResultItemImpl(content.getName(), content
						.extract(String.class), null, new Date(content
						.getTimestamp()));
	...
	/**
	 * From a Passerelle msg with an Attribute in there to one with some other
	 * content type
	 */
	@SuppressWarnings("unchecked")
	public PasserelleToken convertPasserelleMessageContent(
			final PasserelleToken origToken, final Class targetContentType)
			throws UnsupportedOperationException, PasserelleException {
		...
		if (origToken == null || origToken.getMessage() == null
			...
			return origToken;
		} else if (TangoAttribute.class.isAssignableFrom(origToken
				.getMessageContentType())) {
			PasserelleToken result = origToken;
			try {
				final TangoAttribute content = (TangoAttribute) origToken
						.getMessage().getBodyContent();
				if (content != null
						&& !content.getClass().equals(targetContentType)) {
					final Object resultContent = convertAttributeToSimpleTypedObject(
							content, targetContentType);
					final ManagedMessage msg = MessageFactory.getInstance()
							.copyMessage(origToken.getMessage());
					if (String.class.isInstance(resultContent)) {
						msg.setBodyContentPlainText((String) resultContent);
					} else {
						msg.setBodyContent(resultContent,
								ManagedMessage.objectContentType);
					}
