...
public abstract class DynamicPortsActor extends
		com.isencia.passerelle.actor.dynaport.DynamicPortsActor {
	...
	public DynamicPortsActor(final CompositeEntity container, final String name)
	 ...
	 */
	@Override
	protected void changeNumberOfPorts(final int newPortCount,
			final int currPortCount, final PortType portType)
			throws IllegalActionException, IllegalArgumentException {

		if (logger.isTraceEnabled()) {
			logger.trace(getInfo()
					+ " changeNumberOfPorts() - entry - portType : " + portType
					+ " / new nrOfPorts : " + newPortCount);
		}
		switch (getPortConfiguration()) {
		case EQUAL_NUMBER_INPUT_OUTPUT:
			...
			this
					.createDeletePorts(newPortCount, currPortCount,
							PortType.OUTPUT);
	...
	private void createDeletePorts(final int newPortCount,
			final int currPortCount, final PortType portType)
			throws IllegalActionException, IllegalArgumentException {
	...
	protected void deletePort(final String portName)
			throws IllegalActionException {
		// remove the port
		try {
			...
			throw new IllegalActionException(this, e, "failed to remove port "
					+ portName);
		}
	}
	 ...
	 */
	@Override
	protected Port createPort(final String namePrefix, final int index,
			final boolean isInput, final boolean isOutput)
			throws IllegalActionException {

		if (logger.isTraceEnabled()) {
			logger.trace(getInfo() + " createPort() - entry - name : "
					+ namePrefix + " index : " + (index + idxOffsetPort));
		}
		Port aPort = null;
		...
		try {
			...
			if (aPort == null) {
				...
				logger.debug(getInfo() + " createPort() - port " + portName
						+ " will be constructed");
				if (isInput) {
					aPort = PortFactory.getInstance().createInputPort(this,
							portName, getPortModeForNewInputPort(portName),
							null);
					if (expectedContentType != null) {
						aPort
								.setExpectedMessageContentType(expectedContentType);
					}
				} else {
					aPort = PortFactory.getInstance().createOutputPort(this,
							portName);
				}
			...
			throw new IllegalActionException(this, e, "failed to create port "
					+ portName);
		}
		if (logger.isTraceEnabled()) {
