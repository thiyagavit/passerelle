...
public class BasicDirector extends com.isencia.passerelle.domain.cap.Director {
	...
	selectedErrCtrlStrategyParameter.addChoice(ErrorStrategy.DEFAULT.toString());
	selectedErrCtrlStrategyParameter.addChoice(ErrorStrategy.RETRY.toString());
	if (firstTime) {
	    selectedErrCtrlStrategyParameter.setExpression(ErrorStrategy.DEFAULT.toString());
	    firstTime = false;
	}
     ...
     * @throws NameDuplicationException
     */
    public BasicDirector(final Workspace workspace) throws IllegalActionException,
     ...
     * @throws NameDuplicationException
     */
    public BasicDirector(final CompositeEntity container, final String name)
	...
	if (/*getAdapter(null).*/isMockMode()) {
	    ...
	    ExecutionTracerService.trace(this, "###START " + modelName + "### with strategy "
