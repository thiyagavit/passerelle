...
public class EquipmentsSetterWithCommand extends Transformer {
    ...
    public EquipmentsSetterWithCommand(final CompositeEntity container,
	...
	if (!isMockMode()) {
	    try {
		snapManagerName = SoleilUtilities
			.getDevicesFromClass("SnapManager")[0];
		logger.debug(snapManagerName);
		setEquipments = new TangoCommand(snapManagerName,
	...
	if (isMockMode()) {
	    ...
	    try {
		final String snapID = (String) PasserelleUtil
			.getInputValue(arg0);
	...
	if (arg0 == commandNameParam) {
	    ...
	    final String tmp = PasserelleUtil
