...
public class GroupedCommand extends Transformer {
	...
	public GroupedCommand(final CompositeEntity arg0, final String arg1)
		...
		if (!isMockMode()) {
			try {
				if (deviceNames.contains(",")) {
					...
					deviceNameList = FilterHelper
							.getDevicesForPatternAsArray(deviceNames);
				}
				group = new TangoGroupCommand("Command", commandName,
						deviceNameList);
		...
		if (isMockMode()) {
			...
			try {
				...
				if (group.isArginScalar()) {
					// scalar
					final String argin = (String) PasserelleUtil
							.getInputValue(arg0);
					group.insert(argin);
