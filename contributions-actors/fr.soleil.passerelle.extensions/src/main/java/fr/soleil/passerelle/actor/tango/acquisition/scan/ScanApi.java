...
public class ScanApi {
	...
	public void setConfig(final ScanConfiguration config)
	...
	public String getScanRepository() throws InitializationException {
		...
		final Preferences prefs = Preferences
				.userNodeForPackage(SalsaModel.class);
		scanRepository = prefs.get(PARAM_SCANMANAGER_REPOSITORY,
				"not initialized");
	...
	public void loadScanFile(final File file) throws InitializationException {
		...
		try {
			...
			throw new InitializationException("Cannot load salsa config",
					model, e1);
		}
		config.setScanNumber(1);
		scanServer = model.getScanServer();
		...
		// TODO: en attente getDevice retourne une exception!
		if (dev == null) {
			throw new InitializationException("Cannot connect to scan server",
					null, null);
		}
		this.configureDR();
	}
	 ...
	 * @param rangesYList
	 */
	public void changeTrajectory(final Map<Integer, ScanRangeX> rangesXList,
			final Map<Integer, ScanRangeY> rangesYList) {

		if (config.getDimensions().size() >= 1) {
			// first dimension X
			final Dimension dim = config.getDimensions().getDimension(0);
			for (final Map.Entry<Integer, ScanRangeX> config : rangesXList
					.entrySet()) {
				Range salsaRange;
				...
				if (configRange.isIntegrationTimeInit()) {
					((LinearRange) salsaRange).setIntegrationTime(configRange
							.getIntegrationTime());
				}
				// we suppose that there is only one actuator
				final LinearTrajectory traj = (LinearTrajectory) dim
						.getTrajectory(dim.getActuators().getActuator(0),
								salsaRange);
		...
		if (config.getDimensions().size() >= 2) {
			...
			for (final Map.Entry<Integer, ScanRangeY> config : rangesYList
					.entrySet()) {
				Range salsaRange;
				...
				// we suppose that there is only one actuator
				final LinearTrajectory traj = (LinearTrajectory) dimY
						.getTrajectory(dimY.getActuators().getActuator(0),
								salsaRange);
				if (configRange.isRelative()) {
					traj.setRelative(configRange.isRelative());
				}
	...
	public void change1DTrajectory(final double from, final double to,
		...
		final LinearTrajectory traj = (LinearTrajectory) dim.getTrajectory(dim
				.getActuators().getActuator(0), range);
		// we suppose that there is only one actuator
	...
	public void change2DTrajectory(final double xFrom, final double xTo,
			final int xStepNr, final boolean xRelative, final double yFrom,
			final double yTo, final int yStepNr, final boolean yRelative,
		...
		final LinearTrajectory trajY = (LinearTrajectory) dimY.getTrajectory(
				dimY.getActuators().getActuator(0), rangeY);
		// we suppose that there is only one actuator
			...
			if (currentState == DevState.FAULT) {
				...
				if (runName.equals("")) {
					ExecutionTracerService
							.trace(actor,
									"This an known bug in Salsa, cannot do anything :(");
				}
				throw new ProcessingException(dev.status(), dev, null);
			}
		} catch (final DevFailed e) {
			throw new DevFailedProcessingException(e, actor);
		} catch (final ConnectionException ce) {
			throw new ProcessingException(ce.getMessage(), null, ce);
		}
	}
