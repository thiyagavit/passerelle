...
public class ConfigureMonochromator extends ATangoDeviceActor {
	...
	public ConfigureMonochromator(final CompositeEntity arg0, final String arg1)
	...
	protected void doInitialize() throws InitializationException {
		if (!isMockMode()) {
			try {
				...
				configureCmd = new TangoCommand(getDeviceName()
						+ enumDeviceName, confValue);
		...
		try {
			if (isMockMode()) {
				ExecutionTracerService.trace(this,
						"MOCK - Changing monochromator conf to " + confValue);
			} else {
				configureCmd.execute();
				ExecutionTracerService.trace(this,
						"Changing monochromator conf to " + confValue);
			}
			response.addOutputMessage(0, output, PasserelleUtil
					.createCopyMessage(this, message));
