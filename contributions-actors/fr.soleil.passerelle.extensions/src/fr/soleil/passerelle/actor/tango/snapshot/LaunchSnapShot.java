...
public class LaunchSnapShot extends TransformerV3 {
	...
	private final static Logger logger = LoggerFactory
			.getLogger(LaunchSnapShot.class);
	...
	public LaunchSnapShot(final CompositeEntity arg0, final String arg1)
	...
	protected void doInitialize() throws InitializationException {
		...
		if (!isMockMode()) {
			try {
				final String snapManagerName = SoleilUtilities
						.getDevicesFromClass("SnapManager")[0];
				launchCmd = new TangoCommand(snapManagerName, "LaunchSnapShot");
				commentCmd = new TangoCommand(snapManagerName,
						"UpdateSnapComment");
				getSnapIdCmd = new TangoCommand(snapManagerName,
						"GetSnapShotResult");
	...
	protected void process(ActorContext ctxt, ProcessRequest request,
		...
		if (isMockMode()) {
			ExecutionTracerService.trace(this,
					"MOCK - launching snapshot for context " + contextID);
		} else {
			try {
				...
				if (!isFinishRequested()) {
					ExecutionTracerService
							.trace(this,
									"waiting end of state Running on SnapManager finished");
					...
					ExecutionTracerService.trace(this,
							"launching snapshot for context " + contextID);
				} else {
					ExecutionTracerService
							.trace(this,
									"waiting end of state Running on SnapManager cancelled");
				}
			} catch (final DevFailed e) {
		...
		response.addOutputMessage(0, output, PasserelleUtil
				.createContentMessage(this, snapShotID));
	}
	...
	public void attributeChanged(final Attribute arg0)
		...
		if (arg0 == snapShotCommentParam) {
			snapShotComment = PasserelleUtil
					.getParameterValue(snapShotCommentParam);
		} else {
